...
/**
 * clone of class AtlasTmxMapLoader by  Justin Shapcott and Manuel Bua with slight changes to suit
 * kyperbox needs.
 * 
 * @author john
 *
...
public class KyperMapLoader extends BaseTmxMapLoader<AtlasTmxMapLoader.AtlasTiledMapLoaderParameters>{
	...
	@Override
	public Array<AssetDescriptor> getDependencies (String fileName, FileHandle tmxFile, com.badlogic.gdx.maps.tiled.AtlasTmxMapLoader.AtlasTiledMapLoaderParameters parameter) {
	...
	@Override
	public void loadAsync (AssetManager manager, String fileName, FileHandle tmxFile, com.badlogic.gdx.maps.tiled.AtlasTmxMapLoader.AtlasTiledMapLoaderParameters parameter) {
		map = null;
		if (parameter != null) {
			convertObjectToTileSpace = parameter.convertObjectToTileSpace;
			flipY = parameter.flipY;
	...
	@Override
	public TiledMap loadSync (AssetManager manager, String fileName, FileHandle file, com.badlogic.gdx.maps.tiled.AtlasTmxMapLoader.AtlasTiledMapLoaderParameters parameter) {
		if (parameter != null) {
			setTextureFilters(parameter.textureMinFilter, parameter.textureMagFilter);
		}
...
//				}
	...
	protected void loadObject(TiledMap map, MapObjects objects, Element element, float heightInPixels) {
		...
		if (element.getName().equals("object")) {
			...
			object_name = object_name!=null?object_name:has_template?template.getTemplateType().getName():null;
			...
			float height = has_template?template.getHeight()*scaleY:element.getFloatAttribute("height", 0) * scaleY;
			...
			Gdx.app.log(getClass().getSimpleName(), "ObjectName:"+object_name+" template:"+template_name+" x:"+x+" y:"+y+" width:"+width+" height:"+height);
			object.setName(object_name);
			String rotation = element.getAttribute("rotation", null)!=null?element.getAttribute("rotation"):has_template?template.getRotation()+"":null;
			if (rotation != null) {
